...
function prepareData(version, errorCorrectionLevel, encodingHint, chunks) {
    const buffer = new BitBuffer.BitBuffer();
...
function createBytes(buffer, rsBlocks) {
    ...
    for (let r = 0; r < rsLength; r++) {
        ...
        const rawPoly = new Polynomial.Polynomial(dcData[r], ecLength);
...
class Encoder {
    constructor(options = {}) {
        ...
        this.chunks = [];
        this.matrix = [];
        const { version = 0, encodingHint = false, errorCorrectionLevel = ErrorCorrectionLevel.ErrorCorrectionLevel.L } = options;
    ...
    setupCodewords(data, maskPattern) {
        ...
        const maskFunc = MaskPattern.getMaskFunc(maskPattern);
