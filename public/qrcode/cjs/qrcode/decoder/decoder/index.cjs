...
function readCodewords(matrix, version, formatInfo) {
    ...
    const maskFunc = MaskPattern.getMaskFunc(formatInfo.dataMask);
...
function readVersion(matrix) {
    ...
    for (const version$1 of version.VERSIONS) {
        if (version$1.infoBits === topRightVersionBits || version$1.infoBits === bottomLeftVersionBits) {
            return version$1;
        }
        let difference = numBitsDiffering(topRightVersionBits, version$1.infoBits);
...
function decodeMatrix(matrix) {
    ...
    let resultIndex = 0;
    for (const dataBlock of dataBlocks) {
        const correctedBytes = index.rsDecode(dataBlock.codewords, dataBlock.codewords.length - dataBlock.numDataCodewords);
